...
std::size_t Rule::Hash::operator()(const Rule &rule) const {
    return std::hash<char>()(rule.lhs) ^ std::hash<std::string>()(rule.rhs) ^ std::hash<unsigned int>()(rule.index);
}
bool Rule::Compare::operator()(const Rule &r1, const Rule &r2) const {
    return Hash()(r1) < Hash()(r2);
}

Rule::Rule() = default;
Rule::Rule(const Rule &other) : lhs(other.lhs), rhs(other.rhs), index(other.index) {}
Rule::Rule(const std::string line) : lhs(line[0]), rhs(line.substr(3)), index(0) {}
Rule::Rule(const char lhs, const std::string rhs, const unsigned int index) : lhs(lhs), rhs(rhs), index(index) {}
...
bool Rule::operator==(const Rule &other) const {return rhs == other.rhs && index == other.index;}
bool Rule::operator!=(const Rule &other) const {return rhs != other.rhs || index != other.index;}
